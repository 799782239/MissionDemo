...
public class TransferCenter {
    ...
    public void startUserCenter(String visitorId, boolean isOpenMain) {
        ...
        Uri uri = Uri.parse(APP_SCHEME + mContext.getApplicationInfo().packageName).buildUpon()
                ...
                .build();
        Log.i("TransferCenter", visitorId);
        Intent intent = new Intent(Intent.ACTION_VIEW, uri);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        mContext.startActivity(intent);
    }
     ...
     * @param topicType
     */
    public void startMissionDetails(String topicId, String topicType) {
        String pathPrefix = TransferPathPrefix.MISSION_DETAILS;
        if (topicType.equals(TopicType.EVALUATION)
                || topicType.equals(TopicType.THEME)) {
            pathPrefix = TransferPathPrefix.TOPIC_OFFICIAL;
        }
    ...
    public void startChat(String targetId, String title) {
        if (!startLogin()) {
            return;
        }

        Uri uri = Uri.parse("rong://" + mContext.getApplicationInfo().packageName).buildUpon()
                .appendPath(TransferPathPrefix.CHAT)
